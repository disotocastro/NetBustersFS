...
class SistemaArchivosFAT {
 ...
 public:
  ...
  void abrir(const std::string& nombreArchivo, bool verbose);
   void cerrar(const std::string& nombreArchivo, bool verbose = false);
  void crear(const std::string& nombreArchivo, bool verbose = false);
  void crear(const std::string& nombreArchivo, std::vector<char>& datos, bool verbose = false);  // Crear archivo con datos
  void leer(const std::string& nombreArchivo, bool verbose = false);
  void buscar(const std::string& nombreArchivo, bool verbose = false);
  void borrar(const std::string& nombreArchivo, bool verbose = false);
  void adjuntar(const std::string& nombreArchivo, std::vector<char>& datos, bool verbose = false);
  void renombrar(const std::string& nombreArchivo, const std::string& nuevoNombre, bool verbose = false);
  void listar();
  void escribir(const std::string& nombreArchivo, std::vector<char>& datos, bool verbose = false);
 void imprimir();
 private:
  // Método auxiliar para buscar archivos
  int encontrarArchivo(const std::string& nombreArchivo,  bool verbose = false);
